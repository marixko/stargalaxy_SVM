...
abstract: Este tutorial foi criado para a disciplina "Aprendizagem em altas dimensões" (MAE5904 - IME/USP). O exemplo mostrado neste tutorial é de uma classificação supervisionada de estrelas e galáxias.
author: Lilianne Nakazono (IAG-USP)
date: 13/11/2020
...
cat("Resultados da validação cruzada com 2 variáveis preditoras \n")
cv_2_metrics = rbind(cv_2$Fold1, cv_2$Fold2, cv_2$Fold3, cv_2$Fold4, cv_2$Fold5)

cat(">> Média das métricas dos 5 folds: \n")
colMeans(cv_2_metrics)
cat(">> Desvio Padrão das métricas dos 5 folds: \n")
apply(cv_2_metrics, 2, sd)
cat("\n")

cat("Resultados da validação cruzada com 16 variáveis preditoras \n")
...
```
# Extra: Tuning de parâmetros do SVM (K-Fold na amostra de teste)
